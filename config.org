...
这里看看 [[github:tecosaur][@tecosaur]] 对于几个编辑器的比较
...
拉取 doom-emacs 仓库的分支
  - emacs-version: *28.1*
  - git commit: *c44bc81a0*
  - doom-version: *3.0.0-dev*
  - doom-modules-version: *22.08.0-dev*
*** 模组
:properties:
...
*** 启动界面
[[github:tecosaur][tecosaur]] 做了一个相当棒的启动画面，心动！但是太复杂了。我只是想简单的在每次重启时
更换 banner，仅此而已。
...
**** 默认设置
在创建新项目时，我经常希望 remote 到我的个人 gitlab 实例，我希望通过这节内容快速
配置它。

#+begin_src emacs-lisp
(defvar +magit-default-forge-remote (format "git@gitiris:%s/%%s.git" user-full-name)
...
*** iedit
iedit 介绍说它是都行同时编辑工具，当然 lsp 也可能需要它 (如果你要多行同时修改代
码的话)。
...
LaTeX 已经将 =---= 和 =---= 识别为 em/en-dashes，​=\-= 识别为连字符，并且 =...=
到 =\ldots{}= 的转换被硬编码到 ~org-latex-plain-text~​（不像​~org-html-plain-text~​）。

我也很想分辨 =->= 和 =<-=​，所以让我们提出一些建议。
...
# 我们可以编写一个函数，用 =kpsewhich= 检查这些包，然后如果其中任何一个缺失，我们
# 将在生成的配置中注入一些建议，获取缺失包的列表并在每次导出时产生警告。
